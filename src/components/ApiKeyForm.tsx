...
const fadeUp = (delay: number) => ({
  opacity: 0,
  animation: `land-fade-up 0.7s ${EASE} ${delay}s forwards`,
});
const scaleIn = (delay: number) => ({
  opacity: 0,
  animation: `land-scale-in 0.7s ${EASE} ${delay}s forwards`,
});
...
export default function ApiKeyForm({ onSubmit }: { onSubmit: (key: string) => void }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const animate = (now: number) => {
        ...
        el.scrollTop = scrollTarget * ease(progress);
  ...
  return (
    <div ref={scrollRef} className="h-screen flex flex-col overflow-y-auto overflow-x-hidden" style={{ background: "#060606" }}>
      {/* Banner */}
      <div className="text-center py-1.5 text-[11px] font-medium tracking-wide flex-shrink-0" style={{
        ...
        opacity: 0,
        animation: `land-fade-in 0.6s ease 0s forwards`,
      }}>
        Currently only available for Hevy Pro subscribers
      </div>

      {/* Main */}
      <div className="flex flex-col max-w-[1400px] w-full mx-auto px-6">
        {/* Nav */}
        <nav className="flex items-center justify-between py-3 flex-shrink-0" style={fadeUp(0)}>
        ...
        <div className="flex-shrink-0 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-center py-6">
          <div>
            <h1 className="text-[clamp(2rem,4vw,2.8rem)] font-medium leading-[1.08] tracking-tight text-[var(--text-primary)]" style={fadeUp(0.08)}>
              Your training, measured.
            </h1>
            <p className="mt-3 text-[14px] text-[var(--text-secondary)] leading-relaxed max-w-md" style={fadeUp(0.16)}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (key.trim()) onSubmit(key.trim()); }}
              className="mt-5 flex gap-2 max-w-sm" style={fadeUp(0.24)}>
              <input
            ...
            <p className="mt-2 text-[10px] text-[var(--text-muted)]" style={{
              opacity: 0,
              animation: `land-fade-in 0.6s ease 0.32s forwards`,
            }}>Your key stays in your browser. Nothing is stored on a server.</p>
            <div className="flex items-center gap-5 mt-5" style={fadeUp(0.32)}>
          ...
          {/* Body diagrams */}
          <div className="relative hidden lg:flex items-start gap-2" style={scaleIn(0.24)}>
        ...
        <div className="flex-1 min-h-[500px] grid grid-cols-12 gap-3 pb-4">
          {/* Volume chart — left tall */}
          <div className="col-span-4 rounded-2xl p-5 flex flex-col" style={{ ...TILE, ...scaleIn(0.4) }}>
            ...
            <div className="flex-1 flex items-end gap-1.5 min-h-0">
              {VOL_DATA.map((d, i) => {
                ...
                return (
                  <div key={i} className="flex-1 relative rounded-md" style={{
                    ...
                    transform: "scaleY(0)",
                    animation: `land-bar-grow 0.6s ${EASE} ${0.6 + i * 0.06}s forwards`,
                  }} />
                );
              })}
            </div>
          ...
          <div className="col-span-4 grid grid-cols-2 gap-3">
            {[
              ...
              <div key={s.label} className="rounded-2xl p-5 flex flex-col justify-center" style={{
                ...TILE, ...scaleIn(0.48 + i * 0.08),
              }}>
          ...
          {/* PRs — right tall */}
          <div className="col-span-4 rounded-2xl p-5 flex flex-col" style={{ ...TILE, ...scaleIn(0.56) }}>
            ...
            <div className="flex-1 flex flex-col justify-center gap-5">
              {[
                ...
                <div key={pr.name} className="flex items-baseline justify-between" style={fadeUp(0.6 + i * 0.08)}>
                  <div>
          ...
          {/* Chat — bottom wide */}
          <div className="col-span-8 rounded-2xl p-5 flex flex-col" style={{ ...TILE, ...scaleIn(0.64) }}>
            ...
            <div className="flex-1 flex flex-col justify-center gap-4">
              {/* User */}
              <div className="flex items-end gap-3 justify-end" style={fadeUp(0.72)}>
              ...
              {/* Chevy */}
              <div className="flex items-start gap-3" style={fadeUp(0.8)}>
          ...
          <div className="col-span-4 rounded-2xl p-5 flex flex-col" style={{ ...TILE, ...scaleIn(0.72) }}>
                ...
                <div key={m.muscle} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
                    <div className="h-full rounded-full" style={{
                      ...
                      transform: "scaleX(0)",
                      animation: `land-bar-fill 0.6s ${EASE} ${0.9 + i * 0.08}s forwards`,
                    }} />
                  </div>
